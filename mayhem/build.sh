...
# Upstream ships a `rust-toolchain` file pinning `stable`; rustup would honor it and
# override the image's pinned nightly (cargo-fuzz needs nightly for -Zsanitizer).
# Force the image default toolchain via env instead of deleting the file (additivity).
RUSTUP_TOOLCHAIN="$(rustup default | awk '{print $1}')"
...
# Build the project TEST suite with the project NORMAL flags (a clean, non-sanitized
# build) so mayhem/test.sh only RUNS it. Upstream CI runs `cargo test --release`
# over the whole workspace (dhall, dhall_proc_macros, serde_dhall), incl. the
# dhall-lang spec suite driven by dhall/tests/spec.rs (needs the dhall-lang submodule).
echo "=== cargo test --release --no-run (normal flags) ==="
# The dhall-lang submodule may be absent when the build context was a plain clone
# (CI checks it out with submodules: recursive). Fetch it during this first, online
# build; the air-gapped re-run finds it already baked into the image and skips.
if [ ! -e dhall-lang/Prelude ]; then
  git submodule update --init dhall-lang
fi
[ -e dhall-lang/Prelude ] || { echo "ERROR: dhall-lang submodule missing (spec tests need it)" >&2; exit 1; }
env -u RUSTFLAGS cargo test --release --no-run --workspace
